...
/**
 * MapView component for displaying trip destinations on an interactive map
 * @param {Object} props - Component props
 * @param {Array} props.destinations - Array of destination objects to display
...
const MapView = ({ destinations, height = '400px' }) => {
  ...
  /**
   * Calculate the center point from all valid destinations
   * @returns {Array} [latitude, longitude] coordinates for map center
   */
  const getMapCenter = () => {
  ...
  /**
   * Calculate appropriate zoom level based on destination spread
  ...
  /**
   * Create polyline coordinates connecting destinations in sequence order
  ...
  /**
   * Create a custom numbered icon for map markers
   * @param {number} number - The number to display on the marker
  ...
  return (
    <Box sx={{ height, border: '1px solid #ddd', borderRadius: 1, overflow: 'hidden' }}>
      <Box sx={{ p: 1, backgroundColor: '#f8f9fa', borderBottom: '1px solid #ddd' }}>
        <Typography variant="subtitle2" color="text.secondary">
          {validDestinations.length} destination{validDestinations.length !== 1 ? 's' : ''} • Route shown
        </Typography>
      </Box>
      ...
      <Box sx={{ p: 1, backgroundColor: '#f8f9fa', borderTop: '1px solid #ddd' }}>
